...
static void update_menu_items(void) {
	...
	if(sys==SYSTEM1)
		MENU_ITEMS[0] = "当前管理：系统[1]";
	else if(sys==SYSTEM2)
		MENU_ITEMS[0] = "当前管理：系统[2]";
	else
		MENU_ITEMS[0] = "无法识别当前系统";
}
...
void device_toggle_truedualboot(void) {
	...
	sprintf(confirm, "确定 - %s - 双系统共存", enable?"关闭":"打开");

	if (confirm_selection("将会清空所有数据. 确认?", confirm)) {
		// unmount /data
...
int device_get_truedualboot_entry(char* tdb_name) {
	sprintf(tdb_name, "双系统共存 - %s ", dualboot_is_tdb_enabled()?"已打开":"已关闭");
	return 0;
}
...
void device_choose_bootmode(void) {
	int sys = dualboot_select_system("选择系统:");
...
int device_get_bootmode(char* bootmode_name) {
	...
	sprintf(bootmode_name, "当前启动系统: 系统[%d]", sysnum+1);
	return 0;
}
